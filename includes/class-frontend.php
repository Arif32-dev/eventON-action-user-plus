...
class evoaup_frontend{
	...
	public function __construct(){
		...
		// Frontend form
			...
			add_action('evoau_form_before', array($this, 'before_form'), 10);			
		...
		// messages if not qualify for form display
			function before_form(){
				...
				?>
					...
					<p><a class='evcal_btn' href='<?php echo $login_link;?>'><?php evo_lang_e('Login to use purchased submissions');?></a></p>
				<?php endif;?>
	...
	// Woocomerce related functions
		...
		// ORDER Post created on checkout
			function update_order_meta($order_id){
			    ...
			    // old method
			    	if($submission_count>0) update_post_meta($order_id, '_submission_count',$submission_count);
			    }		    
			}
